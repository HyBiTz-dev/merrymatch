...
export default function ChatPage() {
  ...
  const onImagesChange = (image) => {
    setImages(image.target.files[0]);
    setImagesUrl(URL.createObjectURL(image.target.files[0]));
  };
    ...
    if (images) {
      try {
        ...
        socket.current.emit("sendMessages", {
          senderId: state?.id,
          receiverId,
          messages: res.data[0].message_text,
        });
      } catch (error) {
        console.log(error);
      }
      ...
      <main className="flex justify-center bg-main h-[91%]">
        ...
        <div className="bg-bg flex flex-col  gap-16 w-[70.25rem]">
              ...
              <div className="flex flex-col flex-1 justify-end items-center gap-12 relative">
                {messages.length === 0 ? (
                  <div className="relative -top-[77%] w-[46.875.rem] p-6 flex justify-center items-center gap-6 rounded-2xl bg-purple-100 border border-purple-300">
                    <svg
                      className="fill-red-400 stroke-2"
                      width="62"
                      height="36"
                      viewBox="0 0 62 36"
                      fill="none"
                    ...
                    <p className="text-red-700 text-body4">
                      Now you and {user?.name} are Merry Match! <br />
          ...
          <form
            ...
            className={
              images
                ? "flex  border-t border-gray-900 h-[8.25rem] w-full px-14 py-6 items-center"
                : "flex  border-t border-gray-900 h-[6.25rem] w-full px-14 py-6"
            }
          >
                ...
                <div className="flex flex-col w-full">
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute px-2 -top-2 left-16 bg-red-500 hover:bg-red-400 active:bg-red-600 text-white border-none rounded-full shadow-btn1 font-bold"
                    >
                      x
                    </button>
                  </div>
